...
def extract(trades: list[Trade]) -> dict:
    ...
    holds  = [(t.exitAt - t.entryAt).total_seconds() for t in trades]
    return {
        "total":           len(trades),
        "win_rate":        len(wins) / len(trades),
        "avg_adherence":   statistics.mean(t.planAdherence for t in trades),
        ...
        "emotion_dist":    dict(Counter(t.emotionalState for t in trades)),
